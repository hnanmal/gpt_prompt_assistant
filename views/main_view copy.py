...
class MainView(tk.Tk):
    ...
    def update_current_model_label(self):
        ...
        if current:
            self.current_model_var.set(f"🧠 현재 모델: {current}")
        else:
            self.current_model_var.set("🧠 현재 모델: (없음)")
    ...
    def create_widgets(self):
        ...
        self.current_model_label.pack(side="right", padx=10)
    ...
    def on_apply_model(self):
        ...
        if success:
            ...
            self.viewmodel.set_current_model(selected)  # ✅ 현재 모델 저장
            self.update_current_model_label()  # ✅ 라벨 갱신
        else:
